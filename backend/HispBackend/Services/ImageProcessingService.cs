...
public partial class ImageProcessingService
{
    ...
    [LibraryImport("libHISPImageProcessing.so", StringMarshalling = StringMarshalling.Utf16, SetLastError = true)]
    private static partial int image_blur([In] byte[] input, [Out] byte[] output, int width, int height, int kernelRadius);

    [LibraryImport("libHISPImageProcessing.so", StringMarshalling = StringMarshalling.Utf16, SetLastError = true)]
    private static partial int image_gaussian_blur([In] byte[] input, [Out] byte[] output, int width, int height, int kernelRadius, double sigma);
