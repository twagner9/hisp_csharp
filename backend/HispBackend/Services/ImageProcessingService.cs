...
public class ImageProcessingService
{
	...
	private static extern int image_blur(byte[] input, byte[] output, int width, int height, int kernelRadius);

	[DllImport("HISPImageProcessing")]
	private static extern int image_gaussian_blur(byte[] input, byte[] output, int width, int height, int kernelRadius, double sigma);
	...
	public byte[] SimpleBlur(Stream imageStream, int kernelRadius)
	{
		...
		Console.WriteLine($"First RGB: {img.Data[0]}, {img.Data[1]}, {img.Data[2]}");
		...
		Console.WriteLine($"First output RGB: {output[0]}, {output[1]}, {output[2]}");
		if (result != 0)
		{
	...
	public byte[] GaussianBlur(Stream imageStream, int kernelRadius, double sigma)
	{
		...
		byte[] output = new byte[width * height * 3];
		int result = image_gaussian_blur(img.Data, output, width, height, kernelRadius, sigma);
		if (result != 0)
		{
			throw new InvalidOperationException($"Native blur failed with error code {result}");
		}
